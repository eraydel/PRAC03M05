...
class AlbumDrinks : NSObject {
    
    static let instance = AlbumDrinks()
    override private init() {
        super.init()
    }
    
    static let albumName = "DrinksApp"
        ...
        self.checarPermisos { (success) in
            if success {
                if let validAssets = self.assetCollection() { // Album already exists
                    ...
                    PHPhotoLibrary.shared().performChanges({
                        PHAssetCollectionChangeRequest.creationRequestForAssetCollection(withTitle: AlbumDrinks.albumName)
                    })
                    { success, error in
    ...
    private func assetCollection() -> PHAssetCollection? {
        ...
        fetchOptions.predicate = NSPredicate(format: "title = %@", AlbumDrinks.albumName)
